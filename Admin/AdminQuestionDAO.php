...
class AdminQuestionDAO{
	...
	public static function editQuestion($id, $question, $choice_A, $choice_B, $choice_C, $choice_D, $answer){
		global $db;
		try {
			$query = "UPDATE exam SET question = '{$question}', choice_a = '{$choice_A}', choice_b = '{$choice_B}', choice_c = '{$choice_C}', choice_d = '{$choice_D}', answer = '{$answer}' WHERE id = '{$id}' ";
			$result = $db->query($query);
			if ($result) {
	...
	public static function addQuestions($question, $choice_A, $choice_B, $choice_C, $choice_D, $answer){
		global $db;
		try {
			$query = "INSERT INTO exam (question, choice_a, choice_b, choice_c, choice_d, answer) VALUES ('{$question}', '{$choice_A}', '{$choice_B}', '{$choice_C}', '{$choice_D}','{$answer}')";
			$result = $db->query($query);
			if ($result) {
